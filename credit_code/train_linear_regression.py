...
y_pred = clf.predict(X_valid).clip(0,1)
auc = roc_auc_score((y_valid>=0.5).astype(int), y_pred)
ap = average_precision_score((y_valid>=0.5).astype(int), y_pred)
ks = ks_score((y_valid>=0.5).astype(int), y_pred)
print(f"[Linear Regression] AUC={auc:.4f}  AP={ap:.4f}  KS={ks:.4f}")

prob_test = clf.predict(test[cat_cols + num_cols]).clip(0,1)
...
imp_df = pd.DataFrame({"feature": feature_names, "importance": np.abs(coef)}).sort_values("importance", ascending=False).head(20)
plt.figure(figsize=(8,6))
plt.barh(imp_df["feature"], imp_df["importance"])
plt.gca().invert_yaxis()
plt.title("Top 20 Feature Importances (Linear Regression)")
plt.tight_layout()
plt.savefig(os.path.join(OUT, "linear_feature_importance.png"), dpi=200)
plt.close()
print("📊 特征重要性图已保存：linear_feature_importance.png")
